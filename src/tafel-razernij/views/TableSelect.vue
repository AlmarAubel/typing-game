...
<template>
  <div class="max-w-4xl mx-auto p-8 font-roboto">
    <h2 class="text-4xl font-bold text-center mb-12 bg-gradient-to-r from-red-400 to-red-600 bg-clip-text text-transparent">Kies een tafel om te oefenen</h2>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-12">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <label
          :class="[
            ...
            practiceType === 'multiple-choice' ? 'bg-purple-100 border-2 border-purple-500 shadow-md' : 'bg-gray-50 border-2 border-transparent hover:bg-gray-100'
          ]"
        >
          <input type="radio" v-model="practiceType" value="multiple-choice" class="hidden" />
          <div class="flex items-center justify-between w-full">
            ...
            <span
              :class="[
                ...
                practiceType === 'multiple-choice' ? 'border-purple-500 bg-purple-500' : 'border-gray-300'
              ]"
            ></span>
          </div>
        </label>
        ...
        <label
          :class="[
            'flex items-center p-4 rounded-xl cursor-pointer group transition-all duration-200',
            practiceType === 'open' ? 'bg-purple-100 border-2 border-purple-500 shadow-md' : 'bg-gray-50 border-2 border-transparent hover:bg-gray-100'
          ]"
        >
          <input type="radio" v-model="practiceType" value="open" class="hidden" />
          <div class="flex items-center justify-between w-full">
            <span class="flex items-center space-x-3">
            ...
            <span
              :class="[
                'w-4 h-4 rounded-full border-2',
                practiceType === 'open' ? 'border-purple-500 bg-purple-500' : 'border-gray-300'
              ]"
            ></span>
          </div>
        </label>
      ...
      <label class="flex items-center space-x-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all duration-200 cursor-pointer">
        <input type="checkbox" v-model="isRandom" class="w-5 h-5 text-purple-600 rounded border-gray-300 focus:ring-purple-500 focus:ring-2" />
